...
function start() {
    setTimeout(function () {
        ...
        $('.dynagrid-config-form .sortable-visible li[role="option"]').each(function () {
            $dynamic_grid_filter_columns.next().children('ul').append('<li class="list-group-item">' + $(this).text() + ' <div class="material-switch pull-right"><input id="attr' + $(this).attr('id') + '" data-id="' + $(this).attr('id') + '" type="checkbox" checked/><label for="attr' + $(this).attr('id') + '" class="label-primary"></label></div></li>');
            i++;
        });

        $('.dynagrid-config-form .sortable-hidden li[role="option"]').each(function () {
            $dynamic_grid_filter_columns.next().children('ul').append('<li class="list-group-item">' + $(this).text() + ' <div class="material-switch pull-right"><input id="attr' + $(this).attr('id') + '" data-id="' + $(this).attr('id') + '" type="checkbox"/><label for="attr' + $(this).attr('id') + '" class="label-primary"></label></div></li>');
            i++;
        });
