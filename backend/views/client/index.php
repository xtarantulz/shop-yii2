...
<div class="client-index">
    <?= DynaGrid::widget([
        ...
        'gridOptions' => [
            ...
            'panel' => [
                'heading' => "<h3 class='panel-title'><i class='fa fa-user'></i> ".Yii::t('app', 'Таблиця клієнтів')."</h3>",
                'before' => Html::a(Yii::t('app', 'Створити Клієнта'), ['create'], ['class' => 'btn btn-success']),
                'after' => false,
                'showFooter' => false
            ],
            'toolbar' => [
                ...
                [
                    'content' => Html::dropDownList('s_id', null, [1 => Yii::t('app', 'Видалити вибрані')], [
                        ...
                        'onchange' => '
                            var type = $(this).val();
                            if(type > 0){
                                var text = "' . Yii::t('app', 'Ви впевнені, що хочете видалити вибрані клієнта?') . '";
                              
                                krajeeDialog.confirm(text, function(out){
                                    if(out) $.post("' . Url::to(['client/selected']) . '", {type: type, keys: $("#dynagrid-client>div").yiiGridView("getSelectedRows")});
                                });
                            }
                        ',
                    ])
                ...
                'content' => function ($data) {
                    if ($data['status'] == User::STATUS_ACTIVE) {
                        return Html::a('<span class="fa fa-toggle-on"></span>', 'status?id=' . $data['id'], [
                            ...
                            'data-pjax' => 0,
                            'data-method' => 'post',
                            'data-confirm' => Yii::t('app', 'Ви дійсно хочете відключити цього клієнта?')
                        ]);
                    } else {
                        return Html::a('<span class="fa fa-toggle-off"></span>', 'status?id=' . $data['id'], [
                            ...
                            'data-pjax' => 0,
                            'data-method' => 'post',
                            'data-confirm' => Yii::t('app', 'Ви дійсно хочете включити цього клієнта?')
                        ]);
                    }
                },
